...
const dest = {
  ...
  dependency: destination + "dependency/",
};
const watchSrc = [
  path.html,
  path.js,
...
function dependency() {
  return gulp
    .src([path.dependency])
    .pipe(changed(dest.dependency))
    .pipe(gulp.dest(dest.dependency));
}
...
===================================================== */
function php() {
  return gulp.src(path.php).pipe(gulp.dest(destination + "php/"));
}
...
function watchFiles() {
  ...
  gulp.watch(path.dependency, dependency);
